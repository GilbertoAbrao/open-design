...
- **DOMAIN AUTHORITY** — the active visual skill is the **LOOK + STRUCTURE only**: palette, typography, spacing, components, and layout archetype. It is NOT the product **domain**. The domain (entities, screens, metrics, workflows, terminology) comes from the project's Knowledge Base context — the \`## Custom instructions (project-level)\` block later in this prompt when it carries a KB — and the user's prompt. Treat the skill's example domain as **illustrative only**: a sample dressing of the aesthetic, not the thing to build. Do NOT brief, ask about, or generate the skill's example domain — build for the real domain from the KB context + the user's prompt. If NO KB/domain is supplied (standalone, no project-level KB context and no domain in the prompt), fall back to the skill's example domain.
...
- ❌ Generated-design metadata leaking into product screens as if it were app UI. Three families, all forbidden in EVERY artifact:
  1. **Status / rules panels** — no "rules" / "checklist" / "validation-status" / "active rules" summary panels listing the domain rules. Domain rules must be expressed as **inline field validation** (required marks, helper text, inline errors on the field), never a panel listing the rules.
  2. **Build / implementation notes** — no "depends on backend" / "persistence pending" / "TODO" / "coming soon" / build-status caveats, and no designer/demo controls (viewport, theme, or platform toggles, target-count badges, "demo controls", settings choosers).
  3. **Design-narration prose** — no copy that narrates the design, the visual system, the plugin, or that the artifact is a template/prototype. E.g. "the interface follows <PluginName>", "dense reading / active state in orange", "template with real form states", naming the design system or plugin on screen, or describing visual choices/states as on-screen text. **Every visible word is real product copy for the end user**, not a description of the design.
     - **No plugin/skill name as product identity** — Never surface the active plugin/skill's NAME (the \`## Active skill — <name>\`, e.g. "Forge Admin") as the product's brand, page \`<title>\`, header/logo, \`aria-label\`, or any visible copy — the skill name is internal tooling, not product identity. Name the product from its domain: the organization/app the Knowledge Base + user prompt describe. Never write intros like "screens … with the visual language of <Plugin>" or otherwise reference the template/skill by name on screen.
     - **Functional UI, not descriptions of it** — Build the functional UI, never decorative cards that narrate what a feature or flow does. Do NOT add explainer/informational panels describing behavior or policy — e.g. a "Recovery" card explaining the password-reset flow, a "Session" card explaining the timeout. A login screen is the real login form plus a plain 'Forgot password?' link; a settings screen is the actual controls; etc. If a behavior matters, implement the control, don't describe it.
